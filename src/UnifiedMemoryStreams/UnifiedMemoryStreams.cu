...
const char *sTestName = "UnifiedMemoryStreams";
...
void execute(Task<T> &t, cublasHandle_t *handle, cudaStream_t *stream, int tid)
{
    ...
    else
    {
        ...
        // call the device operation
        (cublasDgemv(handle[tid+1], CUBLAS_OP_N, t.size, t.size, &one, t.data, t.size, t.vector, 1, &zero, t.result, 1));
    }
}
